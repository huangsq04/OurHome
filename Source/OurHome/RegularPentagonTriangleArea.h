...
static uint32 VectorHash(const void* InData, int32 Len)
{
...
inline uint64_t crc64(uint64_t crc, const unsigned char *s, uint64_t l) {
	uint64_t j;
...
class OURHOME_API FVectorDouble3D
{
public:
	...
	const FVector ToVector() { return FVector( (float)X,(float)Y,(float)Z ); }
...
FORCEINLINE uint64 GetTypeHash(const FVectorDouble3D& Vector)
{
	return VectorHash(&Vector, sizeof(Vector));
	//return VectorHash(&Vector, sizeof(Vector));
	//return FCrc::MemCrc32(&Vector, sizeof(Vector));
}

class OURHOME_API FVectorDouble2D
{
public:
...
FORCEINLINE uint64 GetTypeHash(const FVectorDouble2D& Vector)
{
...
inline FVectorDouble2D  SphericalFromCartesian(const FVectorDouble3D &InPoint)
{
...
inline FVectorDouble3D  SphericalToCartesian(const FVectorDouble2D &InPoint)
{
...
bool PointInTriangle(const FVectorDouble3D& A, const FVectorDouble3D& B, const FVectorDouble3D& C, 
...
class OURHOME_API FRegularPentagonTriangleArea
{
public:
	FRegularPentagonTriangleArea();

	FRegularPentagonTriangleArea(const FVectorDouble3D &P1, 
	...
	static void CreateBaseDodecahedron(TArray<FRegularPentagonTriangleArea> &OutArray);
