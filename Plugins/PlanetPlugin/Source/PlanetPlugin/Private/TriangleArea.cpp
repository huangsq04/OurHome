...
void FTriangleArea::CreateBaseDodecahedron(TArray<FTriangleArea> &OutArray)
{
	if (OutArray.Num() != 0)
	{
		return;
	...
	for (int i = 0; i < RegularPentagon.Num() / 5; i++)
	{
		...
		OutArray.Add(FTriangleArea(C4, BaseVertices[RegularPentagon[Ix + 1]],
			BaseVertices[RegularPentagon[Ix + 0]], OutArray.Num()));
		OutArray.Add(FTriangleArea(C4, BaseVertices[RegularPentagon[Ix + 2]],
			BaseVertices[RegularPentagon[Ix + 1]], OutArray.Num()));
		OutArray.Add(FTriangleArea(C4, BaseVertices[RegularPentagon[Ix + 3]],
			BaseVertices[RegularPentagon[Ix + 2]], OutArray.Num()));
		OutArray.Add(FTriangleArea(C4, BaseVertices[RegularPentagon[Ix + 4]],
			BaseVertices[RegularPentagon[Ix + 3]], OutArray.Num()));
		OutArray.Add(FTriangleArea(C4, BaseVertices[RegularPentagon[Ix + 0]],
			BaseVertices[RegularPentagon[Ix + 4]], OutArray.Num()));
	}

}
FTriangleArea::FTriangleArea(const FVectorDouble3D &P1,
	...
	CenterPos(P1), Pos0(P2), Pos1(P3), Index(Idx)
{
	...
	SphericalCoordinateCenter = UPlanetFunctionLib::SphericalFromCartesian(P1);
	SphericalCoordinate0 = UPlanetFunctionLib::SphericalFromCartesian(P2);
	SphericalCoordinate1 = UPlanetFunctionLib::SphericalFromCartesian(P3);

	MinAngle = acos(Pos0|Pos1);
}

bool FTriangleArea::BesideArea(const FVectorDouble3D &Pos, float Len)
{
...
bool FTriangleArea::InArea(const FVectorDouble3D &Pos)
{
	return UPlanetFunctionLib::PointInTriangle(CenterPos, Pos0, Pos1, Pos);
}

FVectorDouble3D FTriangleArea::LocalXYToWorldPosition(const FIntPoint &Point, int Level, bool UseRadius)
{
...
FIntPoint FTriangleArea::GetCenter(const FVectorDouble3D &InPoint, const TArray<FIntPoint> &Idx, int Level, int MaxLevel)
{
	int Layer = FMath::Pow(2, Level);
	...
	for (int i = 0; i < InTriangles.Num() / 3; i++)
	{
		...
		bool In = UPlanetFunctionLib::PointInTriangle(VT10, VT11, VT12, InPoint);
...
FIntVector FTriangleArea::GetHexXY(const FVectorDouble3D& Point, int Level)
{
...
FIntVector FTriangleArea::PointInTile(const FVectorDouble3D& Point, int Level)
{
...
FVectorDouble3D FTriangleArea::Point2dToVector3d(const FVector2D &P, const FVectorDouble3D &P1,
...
FVectorDouble3D FTriangleArea::PointFromLineSegment(const FVectorDouble3D &P1,
