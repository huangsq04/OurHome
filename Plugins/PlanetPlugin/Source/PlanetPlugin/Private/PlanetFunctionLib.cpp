...
FVectorDouble2D  UPlanetFunctionLib::SphericalFromCartesian(const FVectorDouble3D &InPoint)
{
	double Theta = acos(InPoint.Z);
	double Phi = atan2(InPoint.Y, InPoint.X);
	if (Phi < 0)
	{
		Phi = 2 * PI + Phi;
	}
...
FVectorDouble3D  UPlanetFunctionLib::SphericalToCartesian(const FVectorDouble2D &InPoint)
{
	double X = sin(InPoint.X) * cos(InPoint.Y);
	double Y = sin(InPoint.X) * sin(InPoint.Y);
	double Z = cos(InPoint.X);
	return { X, Y, Z };
}
uint64_t UPlanetFunctionLib::crc64(uint64_t crc, const unsigned char *s, uint64_t l)
{
	uint64_t j;
...
uint64 UPlanetFunctionLib::GetTypeHash(const FVectorDouble2D& Vector)
{
	return crc64(0, (const unsigned char *)&Vector, sizeof(Vector));
}
uint64 UPlanetFunctionLib::GetTypeHash(const FVectorDouble3D& Vector)
{
	return crc64(0, (const unsigned char *)&Vector, sizeof(Vector));
}
bool UPlanetFunctionLib::VectorsOnSameSide(const FVectorDouble3D& Vec, const FVectorDouble3D& A,
...
bool UPlanetFunctionLib::PointInTriangle(const FVectorDouble3D& A, const FVectorDouble3D& B,
...
double UPlanetFunctionLib::PointToSegDist(const FVectorDouble3D &A, const FVectorDouble3D &B, const FVectorDouble3D &P)
{
...
FQuatDouble UPlanetFunctionLib::GetQuatDouble(const FVectorDouble3D &A, const FVectorDouble3D &B, double AngleRad)
{
...
FVectorDouble3D UPlanetFunctionLib::RotatorVector(const FVectorDouble3D &V, const FQuatDouble &Q)
{
